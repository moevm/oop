...
{public:
    ...
    Composite(std::vector<Shape*> v):vec1(v){}
    void move_all(){
    for(auto  m:this->vec1)
    m->move_by_coordinates(5, 5);}
    void inscrease_all(){
        for(auto  m:this->vec1)
            m->inscrease_by_cf(90);}
    void turn_all(){
        for(auto  m:this->vec1)
            m->turn_by_corner(90);}
...
public:
    ...
    std::vector<  Lsosceles_trapeze>   l_tr   ;
    std:: vector< Trapeze>    tr;
   
};
class Any_Shape:public Builder
{
public:
    Builder * p;
    Any_Shape()  { p = new Builder; }
    void creatreRectangle(){
          Rectangle rec(6,2,std::make_pair(0,0));
    p->rec.push_back(rec);
    }
    void creatreLsosceles_trapeze(){
        Lsosceles_trapeze l_tr(10.0,8,4,std::make_pair(0,0));
        p->l_tr.push_back(l_tr);
    }
    void creatreTrapeze(){
