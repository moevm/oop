...
public:
    ...
    virtual void set_char(int health, int n_a, int f_a) = 0;
    virtual Item* copy() = 0;
...
public:
    ...
    virtual void setHealth(int value){
        if (value < 0)
            throw Limit_Exception("Wrong health", value);
        health = health - value;
    }

    virtual void set_char(int healths, int n_a, int f_a){
        if (healths < 0)
            throw Limit_Exception("Wrong health", healths);
        health = healths;
        near_battle.set_attack(n_a);
        further_battle.set_attack(f_a);
    }
