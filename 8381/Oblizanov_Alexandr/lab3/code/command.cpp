...
GameCommand::GameCommand(UIFacade *facade, Game *game, RequestType type, std::vector<int> param)
...
std::vector<int> GameCommand::exec()
{
    ...
    } else if (type == BASE_INFO)
    {
        if (param.front() >= 0)
        {
            ...
            if (number < game->getBases().size())
            {
                ...
                FieldCommand fCom(facadeMediator, game->getField(), FIND, param, base);
                answer = fCom.exec();
    ...
    {
        FieldCommand fCom(facadeMediator, game->getField(), type, param, nullptr);
        return fCom.exec();
    } else if (type == GETXY)
    {
        FieldCommand fCom(facadeMediator, game->getField(), type, param, nullptr);
...
BaseCommand::BaseCommand(IFacadeMediator *facade, Base *base, RequestType type, std::vector<int> param, FieldItem* toFind)
    : base(base), type(type), param(param), toFind(toFind), facadeMediator(facade) {}
...
FieldCommand::FieldCommand(IFacadeMediator *facade, Field *field, RequestType type, std::vector<int> param, FieldItem *toFind)
    : field(field), type(type), param(param), toFind(toFind), facadeMediator(facade) {}
