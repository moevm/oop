...
Field::~Field()
{
    for (unsigned i=0; i<width; i++)
    {
        for (unsigned j=0; j<height; j++)
        {
            if (items[i][j] != nullptr && items[i][j]->getType() != BASE)
                deleteItem(i, j);
            delete landscapes[i][j];
        }
        delete [] items[i];
        delete [] landscapes[i];
    }
...
std::pair<unsigned, unsigned> Field::getItemCoords(FieldItem *item)
{
    for (unsigned i=0; i<width; i++)
    {
        for (unsigned j=0; j<height; j++)
    ...
    throw std::invalid_argument("Item on field not found");
}
