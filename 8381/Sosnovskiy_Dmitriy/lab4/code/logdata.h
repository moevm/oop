...
public:
    ...
    QString convertToString() override {
        ...
        if(formatType == ACTIONS_ONLY)
        {
            ...
            returnable = "New game has started. Players amount: " + QString::number(data->playersAmount)
                    + ", field rows amount: " + QString::number(data->rows) + ", field columns amount: " + QString::number(data->columns)
                    + ", field maximum entities amount: " + QString::number(data->maximumEntitiesAmount)
