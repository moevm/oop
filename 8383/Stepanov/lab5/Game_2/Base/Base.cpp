...
#include "Base.hpp"
    ...
    Unit* Base::createUnit(int whatUnit, bool isProtected, int uniqNum) {
        ...
        if (whatUnit == 0) u = smo.CreateUnit(uniqNum, isProtected);

        else if (whatUnit == 1) u  = sho.CreateUnit(uniqNum, isProtected);

        else if (whatUnit == 2) u = run.CreateUnit(uniqNum, isProtected);
...
//    ++++++++++++++++++++++++++++++++++++++
    ...
    Base::Base (int i, int x, int y, Field *field, ProxyLog *log) {
        this->x = x;
        this->y = y;
    ...
    bool Base::setFirstUnits (Field *field) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            units[i] = createUnit(arr[i], true, concretCountUnit[arr[i]]+'0');
    ...
    if (whatInCell == 4) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (units[i] == nullptr) {
                ...
                units[i] = createUnit(numOfUnit, isProtected, concretCountUnit[numOfUnit]+'0');
                units[i]->Attach(this);
...
BaseSnapshot* Base::saveBase(){
    ...
    return new BaseSnapshot(names, h, a, p, blocAtt, ux, uy, numberUnit, countUnit, x, y, concretCountUnit, name);
...
void Base::restore (BaseSnapshot* snap, ProxyLog *log, Field *field){
    ...
    for (int i = 0; i < snap->names.size(); i++){
        ...
        if (snap->names[i][(numB+1)%2] == '!'){
            ...
            u = createUnit(2, true, snap->names[i][(numB)%2]);
            
        } else if (snap->names[i][(numB+1)%2] == '<' || snap->names[i][(numB+1)%2] == '>'){
            
            u = createUnit(1, true, snap->names[i][(numB)%2]);
            
        } else if (snap->names[i][(numB+1)%2] == '$'){
            
            u = createUnit(0, true, snap->names[i][(numB)%2]);
