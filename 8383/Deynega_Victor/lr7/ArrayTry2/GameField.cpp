...
//	bx = 0;
//	by = 0;
    this->defBase = nullptr;
    this->attackBase = nullptr;
...
GameField::GameField() {
	...
	field[2 * side + 12].landType = mainLake;
	field[11 * side + 6].landType = mainLake;
	field[14 * side + 4].landType = mainLake;
	field[8 * side + 11].landType = mainLake;
	field[8 * side + 12].landType = mainLake;
	field[9 * side + 10].landType = mainLake;
	field[9 * side + 11].landType = mainLake;
	field[9 * side + 12].landType = mainLake;

    field[5 * side + 1].landType = mainMountain;
    field[5 * side + 7].landType = mainMountain;
    field[10 * side + 1].landType = mainMountain;
    field[9 * side + 4].landType = mainMountain;
    field[8 * side + 5].landType = mainMountain;
    field[5 * side + 7].landType = mainMountain;
    field[7 * side + 10].landType = mainMountain;
}
...
Memento* GameField::createMemento(int health, int turn, int units) {//ne rabotaet ofc
    ...
    message += "1_" + std::to_string(5) + "_" + std::to_string(0) + "_" + std::to_string(health) + "_" + std::to_string(size * size / 2);
...
bool Proxy::proxyChecker(int x, int y) {
    if (gameField->field[x + gameField->size * y].landType->getType() == 0 && gameField->field[x + gameField->size * y].unit == NULL && (x != 5 || y != 0)) {
		return true;
	}
	return false;
}
