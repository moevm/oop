...
: width_(width), height_(height), countObj(0), attackMediator(nullptr)
{
    initialise();
    createMap();
    attackMediator = new AttackMediator(this);
}
...
void GameField::addObj(Obj obj){
    ...
    if (obj->isHero()){
        obj->setMediator(attackMediator);
...
void GameField::deleteObj(int x, int y){
    Obj obj = gridLogic[x][y]->getObj();
    if (obj){
        gridLogic[x][y]->removeObj();
        deleteObj(obj);
    }
}
...
GameObj* GameField::getObj(int x, int y){
    return gridLogic[x][y]->getObj();
}
