...
Base::Base(int hp, int lim)
    : hitPoint(hp), unitLimit(lim), unitCount(0), factory_(nullptr), relation(Event::FRIEND)
{}
...
Event Base::getInfo(){
    Event event;
    event.type = Event::BASE_INFO;
    event.baseInfo.hitPoint = hitPoint;
    event.baseInfo.relation = relation;
    event.baseInfo.unitCount = unitCount;
    event.baseInfo.unitLimit = unitLimit;
    event.baseInfo.canCreate = unitLimit - unitCount;
    event.baseInfo.race = race;
    return event;
}
...
Event::Relation Base::getRelation(){
    return relation;
}
