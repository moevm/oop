...
Field::Field(const Field& f) {      //конструктор копировани¤
...
Field::Field(Field&& f) {                    //конструктор перемещени¤
	this->current1stPlayerUnits = f.current1stPlayerUnits;
	this->current2ndPlayerUnits = f.current2ndPlayerUnits;
	this->size = f.size;
	this->med = f.med;
...
void Field::print() {
	...
	for (int i = 0; i < size; i++) {
		cout << "|";
		for (int j = 0; j < size; j++) {
			if (cell[i][j].getUnit().GetValue() != "---")        //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ ёЌ»“, “ќ ѕ≈„ј“ј≈ћ ≈√ќ
				cout << " " << cell[i][j].getUnit().GetValue();

			else if (cell[i][j].getNeutral() != nullptr) {

				if (cell[i][j].getNeutral()->getNeutObjType() == "HealthR") {        //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ Ќ≈…“–јЋ№Ќџ… ќЅЏ≈ “ “»ѕј 'ѕќѕќЋЌ≈Ќ»≈ «ƒќ–ќ¬№я', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
					cout << " " << "+HP";
				}

				else if (cell[i][j].getNeutral()->getNeutObjType() == "ArmorR") {        //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ Ќ≈…“–јЋ№Ќџ… ќЅЏ≈ “ “»ѕј 'ѕќѕќЋЌ≈Ќ»≈ Ѕ–ќЌ»', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
					cout << " " << "+AR";
				}

				else if (cell[i][j].getNeutral()->getNeutObjType() == "Killer") {        //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ Ќ≈…“–јЋ№Ќџ… ќЅЏ≈ “ “»ѕј '”Ѕ»¬јЋ ј', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
					cout << " " << "/I\\";
				}

				else if (cell[i][j].getNeutral()->getNeutObjType() == "Freezer") {        //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ Ќ≈…“–јЋ№Ќџ… ќЅЏ≈ “ “»ѕј '«јћќ–ќ« ј', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
					cout << " " << "***";
				}
			}
			...
			else if (cell[i][j].getLandscape()->getLandscapeType() == "forest") {      //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ ЋјЌƒЎј‘“ “»ѕј 'Ћ≈—', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
				cout << " " << "~~~";
			}

			else if (cell[i][j].getLandscape()->getLandscapeType() == "burningE") {      //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ ЋјЌƒЎј‘“ “»ѕј '–ј— јЋ≈ЌЌџ≈ «≈ћЋ»', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
				cout << " " << "$$$";
			}

			else if (cell[i][j].getLandscape()->getLandscapeType() == "UnknownL") {      //≈—Ћ» ¬  Ћ≈“ ≈ ≈—“№ ЋјЌƒЎј‘“ “»ѕј 'Ќ≈»«¬≈—“Ќџ≈ «≈ћЋ»', “ќ ѕ≈„ј“ј≈ћ ≈√ќ
				cout << " " << "???";
			}
	...
	cout << "Ћандшафт: ~~~ - лес; $$$ - раскаленные земли; ??? - неизвестные земли\n";
	cout << "Ќейтральные объекты: +HP - пополнение здоровь¤; +AR - пополнение брони; /I\\ - убивалка; *** - заморозка\n";
	cout << "\"###\" - база людей и эльфов, \"#-#\" - база орков\n";
	cout << "¬ведите \"exit\" - дл¤ выхода, \"add\" - дл¤ добавлени¤ персонажа, \"del\" - дл¤ удалени¤ персонажа,\n\"base\" - дл¤ базы, ";
	cout << "\"save\" - дл¤ сохранени¤ игры, \"load\" - дл¤ загрузки игры\n";
	cout << endl;
}
...
void Field::init(int numOfOrcs, int numOfElfs, int numOfMens) {
	...
	int num = 0;   //дл¤ рандомного выбора юнитов
	...
	for (int i = size / 2 + 2; i < size; i++) {					//нижн¤¤ половина
		for (int j = 0; j < size; j++) {
...
void Field::moving(string& character, int direction) {
	...
	case 6:
		...
		*logger << "#UNIT: ёнит \"" << cell[coordx][coordy].getUnit().GetValue() << "\" атакует \"" << w.GetValue() << "\"" << Logger::Endl();
	...
	default:
		cout << "ёнит остаетс¤ на месте\n";
		break;
	}

	if (direction >= 6 && direction <= 9 && w.GetValue() == "---") {
		if (warriorToAttack[0] == 'O') {
			cout << "ѕерсонаж " << warriorToAttack << " умер\n";
		...
		*logger << "#UNIT: ”мирает юнит \"" << warriorToAttack << "\"" << Logger::Endl();
	}
}
...
string Field::check(string& character) {
	...
	if (cell[coordx][coordy].getUnit().GetAbilityToWalk() == 0) {
		...
		if (coordx != 0) {
			if (cell[coordx - 1][coordy].getUnit().GetValue() == tmp) {
				...
				if (cell[coordx][coordy].getUnit().GetValue()[0] == 'M' || cell[coordx][coordy].getUnit().GetValue()[0] == 'E') {	//если люди и эльфы
					...
					else if (cell[coordx - 1][coordy].getUnit().GetValue()[0] != 'M' && cell[coordx - 1][coordy].getUnit().GetValue()[0] != 'E' && 
						cell[coordx - 1][coordy].getUnit().GetValue() != "###") {   //если юнит
						opportunity += '6';
		...
		if (coordy != size - 1) {
			if (cell[coordx][coordy + 1].getUnit().GetValue() == tmp) {
				...
				if (cell[coordx][coordy].getUnit().GetValue()[0] == 'M' || cell[coordx][coordy].getUnit().GetValue()[0] == 'E') {
					...
					else if (cell[coordx][coordy + 1].getUnit().GetValue()[0] != 'M' && cell[coordx][coordy + 1].getUnit().GetValue()[0] != 'E' &&
						cell[coordx][coordy + 1].getUnit().GetValue() != "###") {
						opportunity += '7';
			...
			if (cell[coordx + 1][coordy].getUnit().GetValue() == tmp) {
				...
				if (cell[coordx][coordy].getUnit().GetValue()[0] == 'M' || cell[coordx][coordy].getUnit().GetValue()[0] == 'E') {
					if (cell[coordx + 1][coordy].getUnit().GetValue() == "#-#") {	//если база орков
						...
						cell[coordx + 1][coordy].getUnit().GetValue() != "###") {
						opportunity += '9';
		...
		cout << "ёнит не может ходить, так что единственный вариант: \n5 - не двигатьс¤\n";
	}
...
void Field::unitsSpecAbil(int dir, Warrior w, int x, int y) {
	...
	if (character == "MS") {                                        //јЅ»Ћ ј „≈Ћќ¬≈ ј- ќѕ№≈Ќќ—÷ј
		if (dir == 6 && x >= 2) {
	...
	else if (character == "OD") {                                        //јЅ»Ћ ј ќ– ј-ѕќ∆»–ј“≈Ћя
		srand(time(0));
		int num = 0;
		num = rand() % 2;
		if (num == 1) {
	...
	else if (character == "OR") {                                        //јЅ»Ћ ј ќ– ј-–ј«Ѕќ…Ќ» ј
...
Warrior Field::add(string& character, int coordx, int coordy) {
	Warrior w;
	if (coordx == -30) {
		cout << "¬ведите координаты (сначала x, затем y через enter):\n";
		cin >> coordx;
...
void Field::del(string& character) {
	...
	if (check == 0) {
		cout << "“акого персонажа на поле нет!\n";
		system("pause");
		return;
	}
...
void Field::setNewField(Cell** c) {
	...
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			/*Warrior w = c[i][j].getUnit();
			string character;
			character.push_back(c[i][j].getUnit().GetValue()[0]);
			character.push_back(c[i][j].getUnit().GetValue()[1]);
			ArcherFactory arF;
			SpearmanFactory spF;
			InvisibleFactory inviF;
			WizardFactory wizF;
			...
			if (character == "OD") {
				...
				char sym = dev.GetCounter() + '0';
				dev.SetCounter(dev.GetCounter() + 1);
				w = devF.createUnit(c[i][j].getUnit().GetPlaceX(), c[i][j].getUnit().GetPlaceY(), character + sym);
			}
			else if (character == "OR") {
				current2ndPlayerUnits++;
				char sym = rob.GetCounter() + '0';
				rob.SetCounter(rob.GetCounter() + 1);
				w = robF.createUnit(c[i][j].getUnit().GetPlaceX(), c[i][j].getUnit().GetPlaceY(), character + sym);
			}
			else if (character == "MS") {
				current1stPlayerUnits++;
				char sym = sp.GetCounter() + '0';
				sp.SetCounter(sp.GetCounter() + 1);
				w = spF.createUnit(c[i][j].getUnit().GetPlaceX(), c[i][j].getUnit().GetPlaceY(), character + sym);
			}
			else if (character == "MW") {
				current1stPlayerUnits++;
				char sym = wiz.GetCounter() + '0';
				wiz.SetCounter(wiz.GetCounter() + 1);
				w = wizF.createUnit(c[i][j].getUnit().GetPlaceX(), c[i][j].getUnit().GetPlaceY(), character + sym);
			}
			else if (character == "EA") {
				current1stPlayerUnits++;
				char sym = ar.GetCounter() + '0';
				ar.SetCounter(ar.GetCounter() + 1);
				w = arF.createUnit(c[i][j].getUnit().GetPlaceX(), c[i][j].getUnit().GetPlaceY(), character + sym);
			}
			else if (character == "EI") {
				current1stPlayerUnits++;
				char sym = inv.GetCounter() + '0';
				inv.SetCounter(inv.GetCounter() + 1);
				w = inviF.createUnit(c[i][j].getUnit().GetPlaceX(), c[i][j].getUnit().GetPlaceY(), character + sym);
			}*/

			//cell[c[i][j].getUnit().GetPlaceX()][c[i][j].getUnit().GetPlaceY()].addUnit(w);
