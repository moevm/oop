...
char Base::getTypeOfBase() {
	return typeOfBase;
}

void Base::attackOnBase(int x, int y, int damage, Field& field) {
	if (x == sizeOfField - 1 && y == 0) {	//ME base
		baseHealth -= damage;
	}
	else if (x == sizeOfField - 1 && y == sizeOfField - 1) {	//Orcs base
		baseHealth -= damage;
	}

	if (baseHealth <= 0) {
		if (typeOfBase == 'm') {
			*logger << "#BASE: база людей и эльфов уничтожена" << Logger::Endl();
			string z = "---";
			for (int i = sizeOfField - 4; i < sizeOfField; i++) {
				field.changeCell(z, i, sizeOfField - (sizeOfField - 3));
			}
			for (int j = 0; j < sizeOfField - (sizeOfField - 3); j++) {
				field.changeCell(z, sizeOfField - 4, j);
			}
			vME.clear();
			currentMEnum = 0;
		}
		...
		if (typeOfBase == 'o') {
			*logger << "#BASE: база орков уничтожена" << Logger::Endl();
			string z = "---";
			for (int i = sizeOfField - 4; i < sizeOfField; i++) {
				field.changeCell(z, i, sizeOfField - 4);
			}
			for (int j = sizeOfField - 3; j < sizeOfField; j++) {
				field.changeCell(z, sizeOfField - 4, j);
			}
			vOrcs.clear();
			currentOrcsNum = 0;
...
int Base::getCurrentOrcsNum() {
	return currentOrcsNum;
}

int Base::getCurrentMEnum() {
	return currentMEnum;
}

int Base::getBaseHealth() {
	return baseHealth;
}

vector <Warrior> Base::getOrcsUnits() {
	return vOrcs;
}

vector <Warrior> Base::getMEUnits() {
	return vME;
}

void Base::setOrcsUnits(vector <Warrior> orcs) {
	currentOrcsNum = orcs.size();
	vOrcs = orcs;
}

void Base::setMEUnits(vector <Warrior> ME) {
	currentMEnum = ME.size();
	vME = ME;
}
