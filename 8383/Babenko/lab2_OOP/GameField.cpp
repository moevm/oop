...
void GameField::onUnitAttack(Unit *unit, Unit *other) {
  ...
  TextureProxy landscapeProxy(texture);
  int resDamage = unit->getUnitWeapon().getWeaponDamage() *
      landscapeProxy.getDamageCoef(unit->getUnitWeapon().getWeaponType())
      - other->getUnitArmor().getDamageResistance() *
          landscapeProxy.getAbsorptionCoef(other->getUnitArmor().getArmorType());
