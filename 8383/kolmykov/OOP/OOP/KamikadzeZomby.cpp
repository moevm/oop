...
KamikadzeZomby::KamikadzeZomby(int hp, int maxHP, int ownArmor, int ownDamage, bool visability, int level, int landDamage, int landDefence, std::shared_ptr<std::shared_ptr<ObserverInterface>[]> observers, int observerSize, int observerCount, int coordX, int coordY, std::shared_ptr<Mediator> mediator)
:FatZomby(hp, maxHP, ownArmor, ownDamage, visability, level, landDamage, landDefence, observers, observerSize, observerCount, coordX, coordY, mediator){
	attackBehavior = std::shared_ptr<AttackBehavior>(new BlowingUpAttackBehavior());
	weapon = std::shared_ptr<Weapon>(new BlowingUpHead());
	type = UnitType::KAMIKADZE_ZOMBY;
}
