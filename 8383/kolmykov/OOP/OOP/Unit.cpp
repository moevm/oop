...
Unit::Unit(int hp, int maxHP, int ownArmor, int ownDamage, bool visability, int level, int landDamage, int landDefence, std::shared_ptr<std::shared_ptr<ObserverInterface>[]> observers, int observerSize, int observerCount, int coordX, int coordY, std::shared_ptr<Mediator> mediator) {
	this->hp = hp;
	this->maxHP = maxHP;
...
void Unit::damageUnit(int damage) {
	if (damage - armor->getDefend() > 0) {
		hp -= damage - armor->getDefend();
	}
}
...
{
	return std::shared_ptr<UnitSnapshot>(new UnitSnapshot(type, hp, maxHP, ownArmor, ownDamage, visability, level, landDamage, landDefence, observers, observerSize, observerCount, coordX, coordY));
}
