...
HumanOnBike::HumanOnBike(int hp, int maxHP, int ownArmor, int ownDamage, bool visability, int level, int landDamage, int landDefence, std::shared_ptr<std::shared_ptr<ObserverInterface>[]> observers, int observerSize, int observerCount, int coordX, int coordY, std::shared_ptr<Mediator> mediator)
:HumanOnMachine(hp, maxHP, ownArmor, ownDamage, visability, level, landDamage, landDefence, observers, observerSize, observerCount, coordX, coordY, mediator){
	attackBehavior = std::shared_ptr<AttackBehavior>(new BikeAttack());
	moveBehavior = std::shared_ptr<MoveBehavior>(new BikeMove());
	weapon = std::shared_ptr<Weapon>(new Bat());
	type = UnitType::HUMAN_ON_BIKE;
