...
SpitToEnemyZomby::SpitToEnemyZomby(int hp, int maxHP, int ownArmor, int ownDamage, bool visability, int level, int landDamage, int landDefence, std::shared_ptr<std::shared_ptr<ObserverInterface>[]> observers, int observerSize, int observerCount, int coordX, int coordY, std::shared_ptr<Mediator> mediator)
:SpitZomby(hp, maxHP, ownArmor, ownDamage, visability, level, landDamage, landDefence, observers, observerSize, observerCount, coordX, coordY, mediator){
	attackBehavior = std::shared_ptr <AttackBehavior>(new SpitToEnemyAttack());
	weapon = std::shared_ptr<Weapon>(new SpittingToEnemyHead());
	type = UnitType::SPIT_TO_ENEMY_ZOMBY;
}
