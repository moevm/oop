...
bool Game::moveUnit(int x1, int y1, int x2, int y2, int baseNum) {
...
int Game::getBaseCount() {
    return this->field->baseCount();
}

void Game::printBaseInfo(int ind) {
    Base* base = this->field->getBase(ind);
    std::cout << "health: " << base->getHealth() << '\n';
}
