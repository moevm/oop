...
Iterator::Iterator(int width, int height)
{
	mas = new MapBlock*[width];
	for (int i = 0; i < width; ++i)
		mas[i] = new MapBlock[height];
}

void MapBlock::setUnit(Units* unit)
{
	_unit = unit;
	this->x = unit->XY.x;
	this->y = unit->XY.y;
...
Units* MapBlock::getUnit()
{
	return _unit;
}

/*Iterator::Iterator(int max_X, int max_Y) : m_x(max_X), m_y(max_Y) {}

void Iterator::setMemory()
{
	for (int i = 1; i < m_x; i++) {
		mas[i] = new Units[m_y];
	}
...
Units Iterator::getPointer(int X, int Y)
{
	return *(mas[X*Y]);
}

void Iterator::setObject(int X, int Y, Units* unit)
{
	mas[X*Y] = unit;
}
*/
...
Map::Map(int x, int y) { //конструктор
	...
	width = x;
	height = y;
	iterator = new Iterator(width, height);
//	iterator->setMemory();
