...
public:
    Facade(Ui::MainWindow *ui, unsigned f_game) : ui(ui){
        ...
        err_message.setWindowIcon(QIcon(":/icons/exception.png"));
        adapter = new log::Adapter(flag);
        if (f_game == 1){
    ...
    void add_green_dr(){
        ...
        try {
            ...
            string err_str = err.what();
            err_str += "\nLimit: " + to_string(err.info());
            err_message.setText(QString::fromStdString(err_str));
            err_message.exec();
        } catch (Coord_Error err) {
            string err_str = err.what();
            err_str += "\nx: " + to_string(err.info()[0]) + "   y: " + to_string(err.info()[1]);
            err_message.setText(QString::fromStdString(err_str));
            err_message.exec();
        }
        ui->print_item->setText(QString::fromStdString(print));
    ...
    void save(){
        ...
        for (unsigned int i = 0; i < field->get_height(); i++){
            for(unsigned int j = 0; j < field->get_width(); j++){
                if (field->who_I(i, j) == "B"){
                    s_all.s_bonus.push_back(memento::S_bonus());
                    s_all.s_bonus[k].x = i;
                    s_all.s_bonus[k].y = j;
        ...
        for (unsigned int i = 0; i < field->get_height(); i++){
            for(unsigned int j = 0; j < field->get_width(); j++){
                if (field->who_I(i, j) != "B" && field->who_I(i, j) != "BAS" && field->who_I(i, j) != " "){
                    s_all.s_unit.push_back(memento::S_unit());
                    s_all.s_unit[k].x = i;
                    s_all.s_unit[k].y = j;
        ...
        memento::Memento memento(s_all, 1);
    }
    ...
    void read(){
        ...
        string str = "";
        try {
            memento::Memento memento(s_all, 2);
        ...
        for (unsigned int i = 0; i < s_all.s_unit.size(); i++){
            ...
            try {
                ...
                string err_str = err.what();
                err_str += "\nWrong health: " + to_string(err.info());
                err_message.setText(QString::fromStdString(err_str));
                err_message.exec();
            }
        }
