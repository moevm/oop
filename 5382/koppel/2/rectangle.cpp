...
height(height)
{
	...
	if (width < 0 || height < 0)
	{
		throw std::invalid_argument("Отрицательные стороны не допускаются!!!");
	}
}
...
void Rectangle::print(ostream &ost)
{
	...
	ost << endl << "Прямоугольник" << endl;
	ost << endl << "Текущие координаты: " << 
...
void Rectangle::move(Point new_center)//функция перемещения
{
...
void Rectangle::Scale(double scale) //функция масштабирования
{
	width *= scale;
	height *= scale;
	vertex[0].x = vertex[1].x = current_center.x - width / 2;
...
void Rectangle::rotate(int new_angle)//функция поворота
{
	 // www.abakbot.ru/online-2/91-rotate
