...
public:
    ...
    void assignNext(node<T>* ptr, node<T>* &next){
        if (ptr->abs != NULL){
            ...
            next->abs->secondInput = next->secondResult;
            next->abs->prev = next;
        }
        if (ptr->add != NULL){
            next->add = ptr->add;
            next->add->firstInput = next->firstResult;
            next->add->secondInput = next->secondResult;
            next->add->prev = next;
        }
        if (ptr->neg != NULL){
            next->neg = ptr->neg;
            next->neg->firstInput = next->firstResult;
            next->neg->secondInput = next->secondResult;
            next->neg->prev = next;
        }
        if (ptr->dechun != NULL){
            next->dechun = ptr->dechun;
            next->dechun->firstInput = next->firstResult;
            next->dechun->secondInput = next->secondResult;
            next->dechun->prev = next;
        }
        if (ptr->div != NULL){
            next->div = ptr->div;
            next->div->firstInput = next->firstResult;
            next->div->secondInput = next->secondResult;
            next->div->prev = next;
        }
        if (ptr->divmod != NULL){
            next->divmod = ptr->divmod;
            next->divmod->firstInput = next->firstResult;
            next->divmod->secondInput = next->secondResult;
            next->divmod->prev = next;
        }
        if (ptr->mul != NULL){
            next->mul = ptr->mul;
            next->mul->firstInput = next->firstResult;
            next->mul->secondInput = next->secondResult;
            next->mul->prev = next;
        }
        if (ptr->muldiv != NULL){
            next->muldiv = ptr->muldiv;
            next->muldiv->firstInput = next->firstResult;
            next->muldiv->secondInput = next->secondResult;
    ...
    void CountNext(node<T>* ptr,Ui_MainWindow *mainWindow){
        ...
        qDebug() << ptr->firstInput << "   " << ptr->firstResult;
    }
