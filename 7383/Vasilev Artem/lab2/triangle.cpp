...
Triangle::Triangle(Point& center, Point& p1, Point& p2, Point& p3)
    : center(center), p1(p1), p2(p2), p3(p3)
{
    a = p1.length(p2);
    b = p2.length(p3);
    c = p3.length(p1);
...
{
    print << "Координаты центра: О(" << triangle.center.x << ", " << triangle.center.y
          << ")\nКоординаты вершин: P1(" << triangle.p1.x << ", " << triangle.p1.y << "), "
          << "P2(" << triangle.p2.x << ", " << triangle.p2.y << "), " << "P3(" << triangle.p3.x << ", "
          << triangle.p3.y << ")" << std::endl << "Длины сторон треугольника: a = "
          << triangle.a << ", b = " << triangle.b << ", c = " << triangle.c << std::endl
          << "Текущий ID: " << triangle.global_id << std::endl
          << "Текущий цвет: (" << triangle.color.red << ", " << triangle.color.green << ", " << triangle.color.blue << ")" << std::endl;
    return print;
}
