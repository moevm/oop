...
GameMap::GameMap(int x, int y)
{
	...
	nObj = new NeutralContainer(5);
	landescape = new LandscapeComposite(x, y);
	landescape->GenerateLandscape();
}
...
void GameMap::setUserBase(MapStructure& base_) {
	base = std::make_shared<MapStructure>(base_);
}

void  GameMap::setEnemyBase(MapStructure& base_) {
	enemy = std::make_shared<MapStructure>(base_);
}
