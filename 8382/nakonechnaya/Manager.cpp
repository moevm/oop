        ...
        switch (choice){
            case 1:
                ...
                try {
                    ...
                    std::cout << "File: " << a.getFile();
                    std::cout << "Function: " << a.getFunc();
                    std::cout << "Info: " << a.getInfo();
                    flag = a.getFlag();
                }
                next = true;
                if (!flag) {
    ...
    for (auto & gamer : gameState->bases){
        ...
        for (auto & unit : gamer.second->units){
            ...
            switch (unit->UnitName){
                case 0:
                    gameField->createUnit(HALBERDIER, unit->coordinates->x, unit->coordinates->y, currentBase);
                    break;
                case 1:
                    gameField->createUnit(SKELETON, unit->coordinates->x, unit->coordinates->y, currentBase);
                    break;
                case 2:
                    gameField->createUnit(ARCHER, unit->coordinates->x, unit->coordinates->y, currentBase);
                    break;
                case 3:
                    gameField->createUnit(LICH, unit->coordinates->x, unit->coordinates->y, currentBase);
                    break;
                case 4:
                    gameField->createUnit(GRIFFIN, unit->coordinates->x, unit->coordinates->y, currentBase);
                    break;
                case 5:
                    gameField->createUnit(BONE_DRAGON, unit->coordinates->x, unit->coordinates->y, currentBase);
                    break;
            }
