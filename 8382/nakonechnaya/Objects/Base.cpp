...
void Base::createUnit(FieldCell *cell, UnitName name) {
    ...
    switch (name) {
        case HALBERDIER: {
            auto factoryMethod = new HalberdierFactoryMethod();
            cell->setUnit(factoryMethod->createUnit());
            logString += "halberdier. ";
            break;
        }
        case SKELETON: {
            auto factoryMethod = new SkeletonFactoryMethod();
            cell->setUnit(factoryMethod->createUnit());
            logString += "skeleton. ";
            break;
        }
        case ARCHER: {
            auto factoryMethod = new ArcherFactoryMethod();
            cell->setUnit(factoryMethod->createUnit());
            logString += "archer. ";
            break;
        }
        case LICH: {
            auto factoryMethod = new LichFactoryMethod();
            cell->setUnit(factoryMethod->createUnit());
            logString += "lich. ";
            break;
        }
        case GRIFFIN: {
            auto factoryMethod = new GriffinFactoryMethod();
            cell->setUnit(factoryMethod->createUnit());
            logString += "griffin. ";
            break;
        }
        case BONE_DRAGON: {
            auto factoryMethod = new BoneDragonFactoryMethod();
            cell->setUnit(factoryMethod->createUnit());
            logString += "bone dragon. ";
            break;
        }
        default:
            return;
