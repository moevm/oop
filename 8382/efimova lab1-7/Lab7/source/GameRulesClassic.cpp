...
int GameRulesClassic::checkForEnd() {
	...
	for (int i = 0; i < _users.size(); i++)
		numBases[i] = 0; //все базы изначально в 0
	//проходим по строкам и столбцам карты
	for (int i = 0; i < _board->getBoardProperties().sizeX; i++) { 
		for (int j = 0; j < _board->getBoardProperties().sizeY; j++) {
			auto cell = _board->getCellAt(i, j);//заполн. клетки
			if (!cell.isEmpty) { 
				auto unit = cell.unit;
				auto base = dynamic_cast<PongoBaseBoardUnit*>(unit);// очередность
				if (base != nullptr) {
...
// нач. сост игры - у каждого игрока есть по 1 базе
void GameRulesClassic::initBoard() {
	std::random_device rd;
	std::mt19937_64 mt_rand(rd());
	std::uniform_int_distribution<> x_rand(1, _board->getBoardProperties().sizeX-1);// база рандомно разбросана по карте
	std::uniform_int_distribution<> y_rand(1, _board->getBoardProperties().sizeY-1);
	for (int i = 0; i < _users.size(); i++) {
		auto base = new PongoBaseBoardUnit(_users[i]);
		int x = x_rand(mt_rand);// координаты рандомно разбросаны по карте
