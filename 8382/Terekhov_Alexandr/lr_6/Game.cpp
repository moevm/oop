...
void Game::createPlayerSession() {
    ...
    world->getCell(coordPlayer).setPlayer(playerName, 0, session.playerHP());
    printWorld();
    log.str("");
...
void Game::mainPlay() {
    while (answer != 'q') {
        ...
        if (goFor(coordPlayer))
            for (int _ = 0; _ < session.countUnitSteps(); unitRandomWalk(), ++_);
    }
...
bool Game::goFor(pair<int, int> &coordUnit) {
    ...
    switch (answer) {
        case 'a':
            return goTo(coordUnit, pair{coordUnit.first - 1, coordUnit.second});
            break;
        case 'd':
            return goTo(coordUnit, pair{coordUnit.first + 1, coordUnit.second});
            break;
        case 'w':
            return goTo(coordUnit, pair{coordUnit.first, coordUnit.second - 1});
            break;
        case 's':
            return goTo(coordUnit, pair{coordUnit.first, coordUnit.second + 1});
            break;
...
void Game::menu() {
    while (answer != 'b' && answer != 'u' && answer != 'n' && answer != 's' && answer != 'l' && answer != 'r') {
        cout << "Do you want doing something with (b)ase, (u)nits, (s)ave, (l)oad, (r)estart game or (n)othing?" << endl;
        cin >> answer;
    }
    switch (answer) {
        case 'l': {
...
void Game::attack(Cell &attacker, Cell &defender) {
    ...
    if (!attacker.getUnit()->getIsEnemy() && defender.getIsUnit() && defender.getIsWall()) {
        ...
        if (defender.getBase()->getHealth() < 0 || session.win(defender.getBase()->getUnits().size())) {
            playerName = WINNER;
        }
    } else {
