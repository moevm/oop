...
void Renderer::init_new_game(unsigned width, unsigned height,
                             unsigned players) {
    CommandNewGame(game, width, height, players).execute();
...
void Renderer::render() {
    ...
    escape_handled = true; // TODO disabling escape due to a strange bug
    ...
    if (game_initialised) {
        ...
        if (show_info && contains_point(mouse_pos_real)) {
            if (mouse_coors != selected_coors) {
                draw_tile_info(game->field->tile_at(mouse_coors), BOTTOM_RIGHT);
            }
        }
        ...
        if (selected) {
            ...
            if (selected_tile->get_type() == TILE_BASE &&
                ((Base*)selected_tile)->owner_id == game->current_player_id) {
...
void Renderer::render_field() {
    ...
    for (int x = top_left.x; x < bot_right.x + 1; x++) {
        for (int y = top_left.y; y < bot_right.y + 1; y++) {
            TileInterface* tile = game->field->tile_at({x, y});
            if (tile) {
...
void Renderer::handle_key_presses() {
    ...
    if (IsKeyPressed(KEY_B) && game->current_player->base) {
        Coordinates coors = game->current_player->base->get_coors();
        camera_focus_at(coors);
        set_selected(coors);
    }

    if (IsKeyPressed(KEY_SPACE)) {
        CommandNextPlayer(game).execute();
        selected = false;
        selected_coors = {-1, -1};
    }
}
...
void Renderer::handle_mouse_presses() {
    ...
    if (rmb && selected) {
        if (selected_tile->get_type() == TILE_BASE) {
            ...
            UnitFactory* factory =
                available_units[available_units_idx]->create_unit_factory();
            CommandCreateUnit(game, new_coors, factory).execute();
            delete factory;
            selected = false;
        } else if (selected_tile->get_unit()) {
            CommandUnitAction(game, selected_coors, mouse_coors).execute();
            selected = false;
        }
    }
...
void Renderer::set_selected(Coordinates coors) {
    ...
    selected_tile = game->field->tile_at(coors);

    selected_reachable.clear();
    UnitInterface* unit = selected_tile->get_unit();
    if (selected_tile->get_type() == TILE_BASE)
        selected_reachable = game->field->get_reachable(coors);
    else if (unit && (game->unit_can_move(unit) ||
                      !game->is_owned_by(game->current_player_id, unit)))
