...
FieldProxy::FieldProxy(unsigned width, unsigned height, unsigned seed) {
    field = new Field(width, height, seed);
}
...
unsigned FieldProxy::get_seed() {
    return field->get_seed();
}
...
Coordinates FieldProxy::random_coors() {
    // NOTE can loop forever
    Coordinates coors;
    do {
        coors = field->random_coors();
    } while (!field->is_empty(coors));
    return coors;
}
