...
{
public:
	virtual bool execute(Base * base) = 0;
};
...
public:
	...
	bool execute(Base * base);
	MoveCommand(dirs dir)
	{
		if (dir == Left)
			direction = "left";
		if (dir == Right)
...
private:
	...
	string y;
public:
	AttackCommand(string X, string Y) : x(X), y(Y) { };
	bool execute(Base * base);
};
...
private:
	...
	string number;
public:
	CreateCommand(string who, string num) : unit(who), number(num) { };
	bool execute(Base * base);
};
...
public:
	...
	void delUnit(Unit * p);
	bool volunteer();
	bool isEmpty() { return units.empty(); }
};
class ArmyFactory
{
public:
