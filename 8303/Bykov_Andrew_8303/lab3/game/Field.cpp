...
Field::Field(int w, int h) : width(w), height(h), max_units(w*h/2)
{
	...
	cells[9][2].netral = new Trap();
	cells[9][3].netral = new Rune();
	cells[9][4].netral = new Curse();
	cells[9][5].netral = new Gold();
	cells[6][12].land = new SwampProxy();
	cells[7][12].land = new SwampProxy();
	cells[6][13].land = new SwampProxy();
	cells[7][13].land = new SwampProxy();
	
	cells[2][12].land = new RiverProxy();
	cells[3][12].land = new RiverProxy();
	cells[2][13].land = new RiverProxy();
	cells[3][13].land = new RiverProxy();

	cells[2][16].land = new ForestProxy();
	cells[3][16].land = new ForestProxy();
	cells[2][17].land = new ForestProxy();
	cells[3][17].land = new ForestProxy();
	base.water = new WaterBase(Point(0, 0), Point(2, 2), this);
	base.fire = new FireBase(Point(width - 1, height - 1), Point(width - 3, height - 3), this);
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			if (cells[i][j].unit->getNumber())
			{
				...
				text.setString(cells[i][j].unit->getSnum());
...
bool Field::addUnit(Point p, Unit * unit)
{
	...
	{
		cout << "AddUnit: ERROR Point\n";
		return false;
	}

	if (!(cells[p.y][p.x].unit->isEmpty()))
	{
		cout << "AddUnit: Mesh is occupied\n";
		return false;
	}
...
bool Field::moveUnit(int x1, int y1, int x2, int y2)
{
	...
	{
		cout << "move_unit: ERROR\n";
		return false;
	}
	if (!cells[y2][x2].land->isInteract(cells[y1][x1].unit))
	{
		cout << "move_unit: ERROR this units can't go there\n";
		return false;
	}
	if (!(cells[y2][x2].unit->isEmpty()))
	{
		cout << "move_unit: Mesh is occupied\n";
		return false;
	}
	if (cells[y1][x1].unit->isEmpty())
	{
		cout << "move_unit: No army\n";
