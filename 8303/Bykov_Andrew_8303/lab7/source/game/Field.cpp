...
{
	if (w < 0 || w > 20)
		throw LogicException("Field width", w);
	if (h < 0 || h > 10)
		throw LogicException("Field height", h);
...
bool Field::addUnit(Point p, Unit * unit)
{
	...
	{
		throw LogicException("Unit cords", p.x, p.y);
	}
...
bool Field::addNetral(Point p, Netral * n)
{
	if (!check_point(p.y, p.x))
	{
		throw LogicException("Netral cords", p.x, p.y);
	}
...
{
	if (!check_point(p.x, p.y))
	{
		throw LogicException("Land cords", p.x, p.y);
	}
	cells[p.x][p.y].land = l;
	return true;
}