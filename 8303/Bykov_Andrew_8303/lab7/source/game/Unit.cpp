...
bool Unit::attack(int x, int y)
{
	...
	if (getRange() < distance)
	{
		...
		adapter->failAttack_Unit_distance(this);
		return false;
	}
	if (!properties.enableAttack)
	{
		
		adapter->failAttack_Unit_disable(this);
		return false;
	}
	...
	{
		Base * baseTarget = field->getBuilding(x, y);
		if (baseTarget == nullptr || baseTarget == base)
		{
			return false;
		}
		baseTarget->setHealth(int((baseTarget->getHealth() - getAttack() * properties.debuffAttack * getNumber())));
		return true;
	}
