...
void Snapshot::load()
{
	...
	{
		adapter->loadFailBase();
		throw LogicException("Base properties", health1);
	}
	if (health2 < 0 )
	{
		adapter->loadFailBase();
		throw LogicException("Base properties", health2);
	}
	...
	*field = *newField;
	...
	while (iter != field->end()) {
		...
		else
		{
			...
			throw LogicException("Land sprite", landSprite);
		}
		...
		else
		{
			...
			throw LogicException("Netral sprite", netralSprite);
		}
		...
		if(team != 0 && team != 1)
		{
			...
			throw LogicException("Team number", team);
		}
		if (number < 0)
		{
			throw LogicException("Unit number", number);
		}
...
void Snapshot::loadUnit(string unitSub, int number, int team, int serial, Point dir)
{
	...
	else
	{
		...
		throw LogicException("Unit class", unitSub);
	}
