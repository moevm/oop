...
bool Field::addUnit(Unit *Object, int x, int y, std::string name) {
    auto flag = checkPoint(x,y);
    auto matr = !matrix[x][y].getCharacter();
    auto ch = objectsCounter < maxObjectsOnField;
    auto dd = matrix[x][y].isAvailable(Object);
...
SnapShot* Field::makeSnapShot(std::string mode)
{
    ...
    if(mode == "save")
    {
        tmp = new SnapShot(this, base1, base2, base3, unit1, unit2, adapter, gameMode);
    }
    if(mode == "load")
    {
