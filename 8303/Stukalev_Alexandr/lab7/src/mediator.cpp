...
void Mediator::attack(Unit* unit, int x, int y)
{
    ...
    {
        std::string s1 = std::to_string(x) + " " + std::to_string(y);
        std::string s2 = std::to_string(field->getX()) + "> x && " + std::to_string(field->getY()) +  "> y && x >= 0 && y >= 0";
        throw OutOfRangeExceptions("AttackComm", s1, s2);
    }
}
