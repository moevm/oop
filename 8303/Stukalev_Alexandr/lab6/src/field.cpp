...
Field::Field(int lengthY, int lengthX) :lengthY(10), lengthX(10), max_object_count(1000), obj_count(0),humanBase(nullptr),alienBase(nullptr)
    {
...
{
    Base* base = new Base( max_count, health, this->getX()-1,this->getY()-1, "Alien");
    this->alienBase = base;
    field[base->getY()][base->getX()].AlienBase = base;
}
void Field::create_HBase( int max_count, int health)
{
    Base* base = new Base( max_count, health, 0, 0, "Human");
    this->humanBase = base;
    field[base->getY()][base->getX()].HumanBase = base;
}
...
void Field::create_Landscape(int x, int y, char name)
{
    if( name == 'R' )
        field[y][x].land = new RiverProxy();
    else if( name == 'F' )
        field[y][x].land = new ForestProxy();
    else if( name == 'M' )
        field[y][x].land = new MountainProxy();
    else if( name == 'P' )
        field[y][x].land = new PlainProxy();
}
...
bool Field::checkUnit(int x, int y) const
{
...
char Field::getLandscapeName(int x, int y) const
{
    return this->field[y][x].land->getName();
}

char Field::getNeutralName(int x, int y) const
{
    return this->field[y][x].obj->getName();
}


void Field::insertAUnit(char unit, int health, int damage, int armor, int x, int y)
{
    this->alienBase->insert_unit(this, unit, 'A', health, damage, armor, x, y);
}

void Field::insertHUnit(char unit, int health, int damage, int armor, int x, int y)
{
    this->humanBase->insert_unit(this, unit, 'H', health, damage, armor, x, y);
}

Field& Field::operator=(const Field& field)
{
    this->lengthX = field.lengthX;
    this->lengthY = field.lengthY;
    this->humanBase = field.humanBase;
    this->alienBase = field.alienBase;
    this->field = new Cell* [field.lengthX];
    for (int i = 0; i < lengthY; i++){
        this->field[i] = new Cell[lengthX];
        for (int j = 0; j < lengthX; j++)
...
int Field::getCountofDefA()
{
    return alienBase->getCountofDef();
}

int Field::getCountofDefH()
{
    return humanBase->getCountofDef();
}

bool Field::checkABaseLose()
{
    return  alienBase->checkLose();
}

bool Field::checkHBaseLose()
{
    return  humanBase->checkLose();
}
