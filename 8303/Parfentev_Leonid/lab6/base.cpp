...
Base::canCreateUnit(const std::string &key) const
{
    ...
    if (!FactoryTable::instance()->canCreate(key)) {
        return false;
    }
...
Base::createUnit(const std::string &key, Mediator *m)
{
    ...
    Unit *u = FactoryTable::instance()->create(key);
    int id = addUnit(u);
...
bool
Base::becomeDestroyedBy(Unit *u)
{
    for (auto iter = unitsBegin();
         iter != unitsEnd();
         ++iter) {
        if (iter.unit() == u) {
            return false;
    ...
    for (auto iter = unitsBegin();
         ...
         ++iter) {
        Unit *v = iter.unit();
        v->emit(new events::UnitDeath {v});
    }

    emit(new events::BaseDestroyed {this});
    return true;
}

void
Base::store(std::ostream &os) const
{
    os << "base " << _max_count << " " << _destroyed << "\n";
}

bool
Base::restore(std::istream &is,
              RestorerTable *)
{
    is >> _max_count >> _destroyed;
    return !is.fail();
}
